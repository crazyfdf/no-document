    ...
    <el-col :span="4">
      ...
      <el-menu class="y-menu" :default-openeds="defaultOpeneds" router>
        <el-sub-menu index="components">
          ...
          <el-menu-item v-for="item of components" :key="item.value" :index="item.path">
            {{ item.value }}
          </el-menu-item>
        </el-sub-menu>
      </el-menu>
...
const querySearch = (queryString: string, cb: any) => {
  const results = queryString ? restaurants.value.filter(createFilter(queryString)) : components;
  cb(results);
};
