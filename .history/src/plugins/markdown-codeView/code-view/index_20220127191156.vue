...
export default defineComponent({
  ...
  setup(props) {
    ...
    const demoComponent =
      context[
        `../../../../packages/vue-components/src/components/${props.path}`
      ].default;

    watch(visit, pre => {
      if (pre) {
        const elSnapshot = codeView.value;
        if (elSnapshot) {
          elSnapshot.innerHTML = Prism.highlight(
    ...
    return { codeView, showCodeView, visit, demoComponent };
  }
});
</script>
<style scoped>
