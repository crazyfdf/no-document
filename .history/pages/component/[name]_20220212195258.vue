    ...
    data.value.componentAST.slots.map((item) => {
      ...
      if (item.bindings) {
        item.bindings.forEach((_params) => {
          params.push(`${_params.name}: ${_params.type.name}${_params.description&&`【${_params.description}】``);
        });
      }
      return {
        name: item.name,
    ...
    data.value.componentAST.events.map((item) => {
      ...
      if (item.properties) {
        item.properties.forEach((_params) => {
          params.push(`${_params.name}: ${_params.type.names.join("|")}${_params.description&&`【${_params.description}】`}`);
        });
      }
      return {
        name: item.name,
