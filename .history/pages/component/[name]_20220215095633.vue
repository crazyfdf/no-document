...
const _uiBaseColor = [];
const _uiStatusColor = Object.values(uiColor.status);
const _uiPrimaryColor = Object.values(uiColor.primary);
Object.entries(uiColor).forEach(([key, value]) => {
  if (key !== "status" && key !== "primary") {
    _uiBaseColor.push(value[900] || value["DEFAULT"]);
  }
});
const _uiColor = Object.entries(uiColor).map(([key, value]) => {
  return {
    label: key,
    value: key,
      ...
      if (item.tags?.input) {
        moreProps = {
          ...
          ...item.tags.input[0].description.split(",").reduce((pre, cur) => {
            ...
            return {
              ...pre,
              [key.trim()]: JSON.parse(value.trim()),
            };
          }, {}),
        };
      ...
      if (item.tags?.type) {
        ...
        switch (type) {
          case "date":
            break;
          case "color":
            moreProps = Object.assign(moreProps, { showAlpha: true, predefine: _uiBaseColor });
            break;
          case "colorStatus":
            moreProps = Object.assign(moreProps, { showAlpha: true, predefine: _uiStatusColor });
            break;
          case "colorPrimary":
            moreProps = Object.assign(moreProps, { showAlpha: true, predefine: _uiPrimaryColor });
            break;
          case "font":
            moreProps = Object.assign(moreProps, {
              options: Object.keys(uiFont).map((item) => ({ label: item, value: item })),
            });
            break;
          case "radio":
            item.values =
              item.values || (item.tags.values ? item.tags.values[0].description.split(",") : []);
            moreProps = Object.assign(
              moreProps,
              item.values.length
                ? {
