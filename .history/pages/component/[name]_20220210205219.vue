<template>
  <NuxtLayout name="component-layout">
    <template #header>
      <h1 id="Components">{{ data.componentsAST[0].displayName }}</h1>
    </template>
    <el-row class="y-contents">
      <el-col :span="18">
        ...
        <h3 id="Functions" class="y-slots">Functions</h3>
      </el-col>
      <el-col :span="4">
        <h3>CONTENTS</h3>
        <div v-for="item of contents" :key="item">
          <a :href="`#${item}`">{{ item }}</a>
        </div>
      </el-col>
    </el-row>
...
async function init() {
  data.value = (await useFetch("/api/componentAST", { params: { component: componentName.value } }).data.value;
...
const contents = ["Components", "Props", "Slots", "Emits", "Functions"];
</script>
<style scoped>
.y-props {
  margin-top: 4rem;
