      ...
      <el-col :span="18">
          ...
          <el-table :data="propsData" responsiveLayout="scroll">
            <el-table-column label="Name">
              <template #default="{ row }">
                <div :class="row.required ? 'y-required' : ''">{{ row.name }}</div>
              </template>
            </el-table-column>
        ...
        <template v-if="data.componentAST.slots">
          ...
          <el-table :data="slotsData" responsiveLayout="scroll">
            ...
            <el-table-column label="Params">
              <template #default="{ row }">
                <div v-for="(param, index) in row.params" :key="param">
                  {{ param }}
                </div>
              </template>
            </el-table-column>
        ...
        <template v-if="data.componentAST.events">
          ...
          <el-table :data="emitsData" responsiveLayout="scroll">
            ...
            <el-table-column label="Params">
              <template #default="{ row }">
                <div v-for="param in row.params" :key="params">{{ param }}</div>
              </template>
            </el-table-column>
          </el-table>
        </template>
    ...
    data.value.componentAST.slots.map((item) => {
      ...
      if (item.bindings) {
        item.bindings.forEach((_params) => {
          params.push(`${_params.name}: ${_params.type.name}【${_params.description}】`);
        });
      }
      return {
        name: item.name,
    ...
    data.value.componentAST.events.map((item) => {
      ...
      if (item.properties) {
        item.properties.forEach((_params) => {
          params.push(`${_params.name}: ${_params.type.names.join("|")}【${_params.description}】`);
        });
      }
      return {
        name: item.name,
