<template>
  <NuxtLayout name="component-layout">
    ...
    <h3 class="y-props" v-if="props">Props</h3>
    ...
    <h3 class="y-slots" v-if="slots">Slots</h3>
...
const { data } = await useFetch(`/api/componentAST?component=${componentName.value}`);
const componentAST = data.value.componentsAST[0];
console.log(componentAST);
let { props, slots, displayName, description } = componentAST;
watch(componentName, async () => {
  ({ props, slots, displayName, description } = (
    await useFetch(`/api/componentAST?component=${componentName.value}`)
  ).data.value.componentsAST[0]);
  console.log(props);
});
