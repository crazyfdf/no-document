  ...
  <y-card>
    ...
    <template #content v-if="demoComponent">
      <div class="components" ref="components">
        <component :is="demoComponent" v-bind="componentProps" style="display: inline" />
      </div>
      <y-divider></y-divider>
      <slot></slot>
      <div class="code-view-handler" v-if="code">
        <svg viewBox="0 0 1024 1024" width="22" height="22" class="icon" @click="copyClickHandler">
          <path
...
const demoComponent = computed(() => props.context[props.path].default);
const emit = defineEmits(["importImg"]);
const useClipboard = () => {
...
const copyClickHandler = () => {
  useClipboard().copyText(decodeURIComponent(props.code));
};
const components = ref();
const componentToCanvas = () => {
  // 创建 canvas 标签
  const canvas = document.createElement("canvas");
  // 获取指定的宽高
  const width = parseInt(globalThis.getComputedStyle(components.value).width);
  const height = parseInt(globalThis.getComputedStyle(components.value).height);
  // 宽高扩大 2 倍 处理图片模糊
  canvas.width = width;
  ...
  html2canvas(components.value, options).then((canvas) => {
    // 生成图片地址
    const _imgUrl = canvas.toDataURL("image/png");
    emit("importImg", { img: _imgUrl, width: canvas.width, height: canvas.height });
  });
};
onMounted(() => {
